...
class Simplex_table:
    ...
    def Solve(self) -> np.ndarray:
        ...
        for key in basis.keys():
            solution[basis[key]] = self.matrix[key, self.width-1]
        ...
        for l in range(1, coeff_width):
            ...
            for j in range(coeff_height):
                ...
                if b_j / self.matrix[j, l] < min_solving_value:
                    print("b_j = ", b_j, " self.matrix[j, l] = ", self.matrix[j,l])
                    min_solving_value = b_j / self.matrix[j, l] 
                    min_solving_x = l
                    min_solving_y = j
                    finded = True
